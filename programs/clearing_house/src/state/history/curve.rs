...
pub struct CurveHistory {
    pub head: u64,
    pub deposit_records: [CurveRecord; 32],
}
...
pub struct ExtendedCurveHistory {
    pub head: u64,
    deposit_records: [ExtendedCurveRecord; 1024],
}
...
impl Default for ExtendedCurveHistory {
    fn default() -> Self {
        return ExtendedCurveHistory {
            head: 0,
            deposit_records: [ExtendedCurveRecord::default(); 1024],
        };
    }
}
...
impl ExtendedCurveHistory {
    pub fn append(&mut self, pos: ExtendedCurveRecord) {
        self.deposit_records[ExtendedCurveHistory::index_of(self.head)] = pos;
        self.head = (self.head + 1) % 32;
    }
    ...
    pub fn next_record_id(&self) -> u128 {
        let prev_trade_id = if self.head == 0 { 31 } else { self.head - 1 };
        let prev_trade = &self.deposit_records[ExtendedCurveHistory::index_of(prev_trade_id)];
        return prev_trade.record_id + 1;
    }
}
